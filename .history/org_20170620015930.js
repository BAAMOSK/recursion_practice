...
function orgThis(data) {
  ...
  result.employees = data.map(person =>  {
    if(person.boss === 'Bosworth')// || person.boss === 'Zhao' || person.boss === 'VanDyck' || person.boss ==='Swain' || person.boss === 'Goler' || person.boss === 'Hernandez' || person.boss === 'Moissinac' || person.boss 'Kelley') {
      return person;
    }
  }).filter(Boolean);
